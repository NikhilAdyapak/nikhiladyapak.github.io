...
/* --- Journey map: a winding path drawn through career milestones,
       animated by scroll, with a traveling orb and nodes that light up --- */
const jmap = document.getElementById('jmap');
const jmSvg = document.getElementById('jmSvg');
const jmPath = document.getElementById('jmPath');
const jmPathBase = document.getElementById('jmPathBase');
const jmPathGlow = document.getElementById('jmPathGlow');
const jmOrb = document.getElementById('jmOrb');
let jmLen = 0;
let jmNodes = [];
let jmNodeFracs = [];

function buildJourneyMap() {
    if (!jmap || !jmPath) return;
    jmNodes = Array.from(jmap.querySelectorAll('.jm-node'));
    const crect = jmap.getBoundingClientRect();
    if (crect.width === 0) return;

    jmSvg.setAttribute('viewBox', `0 0 ${crect.width} ${crect.height}`);
    jmSvg.setAttribute('width', crect.width);
    jmSvg.setAttribute('height', crect.height);

    // Anchor points: the center of each milestone dot
    const pts = jmNodes.map(node => {
        const dot = node.querySelector('.jm-dot');
    ...
    // Build an S-curve snaking through the stops
    const swing = Math.min(crect.width * 0.18, 190);
    let d = `M ${pts[0].x} ${pts[0].y}`;
    for (let i = 1; i < pts.length; i++) {
        const p0 = pts[i - 1];
        const p1 = pts[i];
        const s = (i % 2 === 0 ? -1 : 1) * swing;
        const dy = (p1.y - p0.y) * 0.42;
        d += ` C ${p0.x + s} ${p0.y + dy}, ${p1.x + s} ${p1.y - dy}, ${p1.x} ${p1.y}`;
    }
    [jmPathBase, jmPathGlow, jmPath].forEach(p => p.setAttribute('d', d));

    jmLen = jmPath.getTotalLength();
    [jmPath, jmPathGlow].forEach(p => {
        p.style.strokeDasharray = jmLen;
        p.style.strokeDashoffset = jmLen;
    });

    // Fraction of total path length at which each stop sits (y grows monotonically)
    jmNodeFracs = pts.map(pt => {
        let lo = 0, hi = jmLen;
        for (let k = 0; k < 22; k++) {
            const mid = (lo + hi) / 2;
            if (jmPath.getPointAtLength(mid).y < pt.y) lo = mid; else hi = mid;
        }
        return lo / jmLen;
    });
...
function updateJourneyMap() {
    ...
    if (reduceMotion) {
        ...
        prog = Math.max(0, Math.min(1, (window.innerHeight * 0.78 - r.top) / r.height));
    }

    const offset = jmLen * (1 - prog);
    jmPath.style.strokeDashoffset = offset;
    jmPathGlow.style.strokeDashoffset = offset;

    const pt = jmPath.getPointAtLength(jmLen * prog);
    jmOrb.setAttribute('transform', `translate(${pt.x}, ${pt.y})`);
    jmOrb.setAttribute('opacity', (prog > 0.005 && prog < 0.997) ? '1' : '0');

    jmNodeFracs.forEach((frac, i) => {
        jmNodes[i].classList.toggle('lit', prog >= frac - 0.012);
    });
}

if (jmap) {
    window.addEventListener('scroll', updateJourneyMap, { passive: true });
    window.addEventListener('resize', buildJourneyMap);
    window.addEventListener('load', () => setTimeout(buildJourneyMap, 120));
    document.addEventListener('DOMContentLoaded', () => setTimeout(buildJourneyMap, 300));
    if (document.fonts && document.fonts.ready) {
        document.fonts.ready.then(() => buildJourneyMap());
    }
}
...
/* --- Reveal on scroll --- */
const revealEls = document.querySelectorAll('.reveal, .reveal3d');
...
['copyEmail', 'copyEmailBig', 'copyEmailJourney'].forEach(id => {
